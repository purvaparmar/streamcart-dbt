...
RIGHT(
    REGEXP_REPLACE(DATA:customer.phone::STRING,'[^0-9]',''),
    10
) AS phone,
...
{{ clean_amount("DATA:order.total_amount::STRING") }} AS order_total,
...
CASE
    WHEN item:discount_pct::FLOAT > 60 THEN NULL
    ELSE item:discount_pct::FLOAT
END AS discount_pct,
...
WHERE LOWER(is_test_event) <> 'true'
...
AND _LOADED_AT >
(
    SELECT MAX(existing._LOADED_AT)
